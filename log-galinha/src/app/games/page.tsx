...
export default function GamesPage() {
    ...
    const handleSearch = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setResults(data);
            if (data.length === 0) setMessage('No games found.');
        } catch (err: any) {
    ...
    const handleSave = async (game: ExternalMediaResult) => {
        setMessage('');
        try {
            ...
            setMessage(`Successfully saved "${game.title}" to database!`);
        } catch (err: any) {
            setMessage(err.message);
        }
    };
