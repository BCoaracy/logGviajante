...
export default function BooksPage() {
    ...
    const handleSearch = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setResults(data);
            if (data.length === 0) setMessage('No books found.');
        } catch (err: any) {
    ...
    const handleSave = async (book: ExternalMediaResult) => {
        setMessage('');
        try {
            ...
            setMessage(`Successfully saved "${book.title}" to database!`);
        } catch (err: any) {
            setMessage(err.message);
        }
    };
    ...
    return (
        <div className="media-page-container">
            <div className="media-header">
                <h1>Search Books (Google Books)</h1>
            </div>
